...
for(_i=0; _i<MAX_GOB1; _i++)
{
    ...
    {
        if(!argument0 
        &&  is_ancestor_(object_index,Snaraa,Ganon) 
        &&  state )
        {
            break;
        }
...
for(_i=ds_list_size(g.go_mgr.dl_gob2)-1; _i>=0; _i--)
{
    ...
    with(g.go_mgr.dl_gob2[|_i])
    {
        if(!argument0 
        &&  state 
        &&  is_ancestor_(object_index,Elevator,LoDoA,BarrA,Barrier2,Torch,Zelda,NPC_0,BlckB,Switch,PushA,ArenaGate) )
        {   // Allow certain GO to continue drawing
            continue;
        }
